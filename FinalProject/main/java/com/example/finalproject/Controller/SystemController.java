...
public class SystemController {
    ...
    @PostMapping("/system")
    public ResponseEntity addProblemType(@RequestBody  System system, @AuthenticationPrincipal User user){
        systemService.addProblemType(system ,user);
        return ResponseEntity.status(201).body(new ApiResponse("Problem Type added !",201));
    }
    @PostMapping("/systems")
    public ResponseEntity addProblemDiscribe(@RequestBody  System system, @AuthenticationPrincipal User user){
        systemService.addProblemDiscribe(system,user);
